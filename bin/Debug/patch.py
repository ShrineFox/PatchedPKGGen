...
def detect_xdelta():
    try:
        rc = subprocess.run("xdelta3.exe -V", stderr=subprocess.DEVNULL)
        return True
...
def apply_patches(path_in, path_out, apply):
    subprocess.run(f"xdelta3.exe -vfn merge {' '.join(apply)} ./tmp.xdelta")
    subprocess.run(f"xdelta3.exe -vfn -d -s {path_in} ./tmp.xdelta {path_out}")
